// Islamic Law of Inheritance System
...
void printError(const char* message) {
    ...
    for (int i = 0; i < sideSpaces; i++) printf(" ");
    printf("â•‘  \033[1;31m%-*s\033[0;33;43m\033[0;33;43m  â•‘\n", boxWidth - 6, message);
...
void printTableHeader() {
    ...
    for (int i = 0; i < sideSpaces; i++) printf(" ");
    printf("â”‚ HEIR                               â”‚ SHARE TYPE         â”‚ AMOUNT       â”‚\n");
...
void printResidueDistribution(const char* title, float amount, const char* details) {
    ...
    char amountLine[70];
    snprintf(amountLine, sizeof(amountLine), "Amount: %.2f", amount);
    for (int i = 0; i < sideSpaces; i++) printf(" ");
    printf("â•‘ \033[1;36m%-*s\033[0;33;43m â•‘\n", 61, amountLine);  
...
int main() {
    ...
    int gender, spouse, nSpouse, nChild, son, daughter;
    ...
    float wifeShare, husbandShare, motherShare, fatherShare, daughterFixedShare;
    float uterineShare, maternalGrandmotherShare;
    float remaining, totalParts, sonPart, daughterPart;
    float wealth;
    int totalHeirs = 0;
    ...
    do {
        ...
        if (scanf("%f", &wealth) != 1) {
            ...
            while (getchar() != '\n');
            printError("ERROR: Please enter a valid number");
            wealth = -1; 
        } else if (wealth <= 0) {
    ...
    if(totalHeirs == 0 && extendedRelatives == 0) {
        ...
        printSeparator();
        printf("\nTotal wealth: %.2f goes to Islamic Treasury for public welfare\n", wealth);
        return 0;
    }
    ...
    if(gender == 1) {
        ...
        if(nSpouse > 0) {
            if(nChild > 0) {
                wifeShare = wealth / 8;
                printf("Wife(s) share (1/8): %.2f", wifeShare);
            } else {
                wifeShare = wealth / 4;
                printf("Wife(s) share (1/4): %.2f", wifeShare);
            }
            if(nSpouse > 1) {
                printf(" (Each wife: %.2f)", wifeShare / nSpouse);
            }
            printf("\n");
        ...
        if(spouse == 1) {
            if(nChild > 0) {
                ...
                printf("Husband share (1/4): %.2f\n", husbandShare);
            } else {
                husbandShare = wealth / 2;
                printf("Husband share (1/2): %.2f\n", husbandShare);
            }
    ...
    if (nChild > 0 || nSiblings >= 2 || (uterineBrothers + uterineSisters) >= 2) {
        ...
        printf("Mother share (1/3 of residue): %.2f\n", motherShare);
    } 
    else {
        motherShare = wealth / 3;
        printf("Mother share (1/3): %.2f\n", motherShare);
    }
    ...
    if(daughter > 0 && son == 0) {
        if(daughter == 1) {
            ...
            printf("Daughter share (1/2): %.2f\n", daughterFixedShare);
        } else {
            daughterFixedShare = (wealth * 2) / 3;
            printf("Daughters share (2/3): %.2f (Each daughter: %.2f)\n", 
    ...
    if(uterineBrothers + uterineSisters > 0) {
        if(uterineBrothers + uterineSisters == 1) {
            ...
            printf("Uterine sibling share (1/6): %.2f\n", uterineShare);
        } else {
            uterineShare = wealth / 3;
            printf("Uterine siblings share (1/3): %.2f (Each gets: %.2f)\n", 
    ...
    if(fixedShares > wealth) {
        ...
        printf("Fixed shares total (%.2f) exceeds estate (%.2f)\n", fixedShares, wealth);
        printf("Applying proportional reduction:\n\n");
        
        if(wifeShare > 0) {
            wifeShare = (wifeShare * wealth) / fixedShares;
            printf("Wife(s) adjusted share: %.2f", wifeShare);
            if(nSpouse > 1) {
                printf(" (Each wife: %.2f)", wifeShare / nSpouse);
            }
            printf("\n");
        ...
        if(remaining > 0) {
            if(son > 0) {
                ...
                printResidueDistribution("Sons and daughters share residue", remaining, 
                    ...
                    //Green
                printf("    Each son gets: \033[1;32;43m%.2f\033[0;33;43m (Total for %d sons: \033[1;32;43m%.2f\033[0;33;43m)\n", 
                        sonPart, son, sonPart * son);
                if(daughter > 0) {
                    printf("    Each daughter gets: \033[1;32;43m%.2f\033[0;33;43m additional (Total for %d daughters: \033[1;32;43m%.2f\033[0;33;43m)\n", 
    ...
    printf("Maternal grandmother's total: %.2f (fixed) + %.2f (residue) = \033[1;32;43m%.2f\033[0;33;43m\n",
    ...
    float eachShare = remaining / totalUterine;

    printf("Each uterine sibling gets: \033[1;32;43m%.2f\033[0;33;43m\n", eachShare + uterineShare/totalUterine);
}
    ...
    printf("Mother's total share: %.2f (fixed) + %.2f (residue) = \033[1;32;43m%.2f\033[0;33;43m\n",
    ...
    printf("Husband's total share: %.2f (fixed) + %.2f (residue) = \033[1;32;43m%.2f\033[0;33;43m\n",
    ...
    if(nSpouse == 1) {
        printf("Wife's total share: %.2f (fixed) + %.2f (residue) = \033[1;32;43m%.2f\033[0;33;43m\n",
               wifeShare, remaining, wifeShare + remaining);
    } else {
        printf("Total wives' share: %.2f (fixed) + %.2f (residue) = \033[1;32;43m%.2f\033[0;33;43m\n",
               wifeShare, remaining, wifeShare + remaining);
        printf("Each wife gets: \033[1;32;43m%.2f\033[0;33;43m\n", (wifeShare + remaining) / nSpouse);
    }
}
                ...
                printf("Father's total share: %.2f (1/6) + %.2f (residue) = \033[1;32;43m%.2f\033[0;33;43m\n", 
                ...
                if(totalParts > 0) {
                    float brotherPart = (remaining * 2) / totalParts;
                    float sisterPart = remaining / totalParts;
                    
                    printResidueDistribution("Full siblings share residue", remaining, 
                        "Each brother gets 2x each sister's share");
                    if(brotherFull > 0) {
                        printf("    Each full brother gets: \033[1;32;43m%.2f\033[0;33;43m (Total: \033[1;32;43m%.2f\033[0;33;43m)\n", 
                                brotherPart, brotherPart * brotherFull);
                    }
                    if(sisterFull > 0) {
                        printf("    Each full sister gets: \033[1;32;43m%.2f\033[0;33;43m (Total: \033[1;32;43m%.2f\033[0;33;43m)\n", 
    ...
    if (sisterFull == 1) {
        float sisterShare = wealth / 2;
        printResidueDistribution("Full sister gets fixed share (1/2)", sisterShare, "No residuary heir");
        printf("Full sister's share: \033[1;32;43m%.2f\033[0;33;43m\n", sisterShare);
    } else {
        float totalShare = (wealth * 2) / 3;
        float each = totalShare / sisterFull;
        printResidueDistribution("Full sisters get fixed share (2/3)", totalShare, "Divided equally, no residuary heir");
        printf("Each sister gets: \033[1;32;43m%.2f\033[0;33;43m (Total: \033[1;32;43m%.2f\033[0;33;43m)\n", each, totalShare);
    }
}
    ...
    printf("Each paternal uncle gets: \033[1;32;43m%.2f\033[0;33;43m (Total for %d uncles: \033[1;32;43m%.2f\033[0;33;43m)\n",
    ...
    if (totalFixedExcludingSpouse > 0) {
        if (mother == 1) {
            float extra = (motherShare * remaining) / totalFixedExcludingSpouse;
            printf("Mother receives Radd: \033[1;32;43m%.2f\033[0;33;43m â†’ Total: \033[1;32;43m%.2f\033[0;33;43m\n", extra, motherShare + extra);
        }
        if (father == 1) {
            float extra = (fatherShare * remaining) / totalFixedExcludingSpouse;
            printf("Father receives Radd: \033[1;32;43m%.2f\033[0;33;43m â†’ Total: \033[1;32;43m%.2f\033[0;33;43m\n", extra, fatherShare + extra);
        }
        if (maternalGrandmother == 1) {
            float extra = (maternalGrandmotherShare * remaining) / totalFixedExcludingSpouse;
            printf("Maternal grandmother receives Radd: \033[1;32;43m%.2f\033[0;33;43m â†’ Total: \033[1;32;43m%.2f\033[0;33;43m\n", extra, maternalGrandmotherShare + extra);
        }
        if (uterineBrothers + uterineSisters > 0) {
            int totalUterine = uterineBrothers + uterineSisters;
            float extra = (uterineShare * remaining) / totalFixedExcludingSpouse;
            float eachExtra = extra / totalUterine;
            printf("Each uterine sibling receives Radd: \033[1;32;43m%.2f\033[0;33;43m (added to fixed)\n", eachExtra);
        }
        if (daughter > 0 && son == 0) {
            float extra = (daughterFixedShare * remaining) / totalFixedExcludingSpouse;
            float eachExtra = extra / daughter;
            printf("Each daughter receives Radd: \033[1;32;43m%.2f\033[0;33;43m (added to fixed)\n", eachExtra);
        }
    } else {
        printError("No eligible heirs for Radd â€” check fiqh logic");
    }
}
